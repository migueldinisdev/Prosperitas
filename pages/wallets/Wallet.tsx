...
export const WalletDetail: React.FC<Props> = ({ onMenuClick }) => {
    ...
    const toDisplayCurrency = (amount: number, currency: Currency) => {
        const eurValue =
            currency === Currency.EUR ? amount : amount * fxRates[currency];
        return displayCurrency === Currency.EUR
            ? eurValue
            : eurValue / fxRates[displayCurrency];
    };
    ...
    const filteredHoldings = useMemo(() => {
        ...
        return [...rows].sort((a, b) => {
            if (holdingSort === "ticker")
                return a.ticker.localeCompare(b.ticker);
            if (holdingSort === "allocation")
    ...
    return (
        <div className="pb-20 w-full overflow-x-hidden">
            <div className="sticky top-0 z-30 bg-app-bg/80 backdrop-blur-md border-b border-app-border px-4 sm:px-6 py-4 flex items-center gap-4">
                <Link
                    to="/wallets"
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    ...
                    <Card className="p-6">
                        <div className="space-y-4">
                            ...
                            <div className="flex items-start justify-between pt-4 border-t border-app-border">
                                <div>
                                    <p className="text-xs text-zinc-500 uppercase tracking-wider font-semibold">
                                        Invested
                                    </p>
                                    <p className="text-lg font-semibold text-zinc-300 mt-1">
                                        {formatCurrency(42030)}
                                    </p>
                                </div>
                                <div className="text-right">
                                    ...
                                    <div className="flex items-center justify-end gap-1 mt-1 text-app-success">
                                        <ArrowUpRight size={16} />
                                        <span className="text-lg font-semibold">
                                            +{formatCurrency(3200.5)}
                                        </span>
                                    </div>
                                    <p className="text-xs text-app-success/70 mt-0.5">
                                        +7.61%
                                    </p>
                                </div>
                            </div>
                        ...
                        action={
                            <div className="flex flex-col sm:flex-row gap-2 w-full">
                                <select
                                    ...
                                    className="bg-zinc-900 border border-app-border text-sm text-white rounded-lg px-3 py-2 w-full sm:w-auto"
                                >
                            ...
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-app-border">
                                    {filteredHoldings.map((holding) => {
                                        const displayValue = toDisplayCurrency(
                                            holding.value,
                                            holding.currency
                                        );
                                        return (
...
const AddDividendModal: React.FC<BaseModalProps> = ({ isOpen, onClose }) => {
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title="Add Dividend or Interest"
            size="lg"
        >
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
const AddCashModal: React.FC<BaseModalProps> = ({ isOpen, onClose }) => {
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title="Deposit or Withdraw Cash"
            size="lg"
        >
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
