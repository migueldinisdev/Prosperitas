    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Add Trade" size="lg">
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          {/* Value Summary */}
          <div className="bg-zinc-900 border border-app-border rounded-lg p-4 space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white font-medium">{stockValue} {stockCurrency}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              {(fees || tax) && (parseFloat(fees || '0') > 0 || parseFloat(tax || '0') > 0) && (
                <>
                  <div className="border-t border-zinc-800 my-2"></div>
                  <div className="flex justify-between text-sm font-semibold">
                    <span className="text-zinc-300">Net Amount:</span>
                    <span className="text-white">
                      {(parseFloat(fxConvertedValue) - parseFloat(fees || '0') - parseFloat(tax || '0')).toFixed(2)}
                    </span>
                  </div>
                </>
              )}
          ...
          {operation === 'SELL' && (
            <div className="bg-zinc-900 border border-amber-500/20 rounded-lg p-4">
              <p className="text-xs font-semibold text-amber-400 mb-2">Profit Estimation</p>
              <p className="text-xs text-zinc-400 leading-relaxed">
                Profit = (Units × Sell Price × FX Rate) - (Units × Cost Average) - Fees - Tax
              </p>
            </div>
          )}
