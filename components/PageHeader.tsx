    ...
    <header className="sticky top-0 z-30 bg-app-bg/80 backdrop-blur-md border-b border-app-border px-6 py-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button 
            onClick={onMenuClick}
            className="lg:hidden p-2 -ml-2 text-app-text-secondary hover:text-app-text-primary rounded-lg hover:bg-app-card/50"
          >
            <Menu size={20} />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-app-text-primary tracking-tight">{title}</h1>
            {subtitle && <p className="text-sm text-app-text-secondary mt-0.5">{subtitle}</p>}
          </div>
        </div>
        <div className="flex items-center gap-4">
