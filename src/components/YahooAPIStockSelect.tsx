...
import {
    fetchYahooStockSearch,
    YahooStockSearchResult,
} from "../data/api/prices/yahooStockSearchApi";
...
const formatOptionLabel = (option: YahooStockSearchResult) => {
    const parts = [
        option.symbol,
        option.name,
        option.exchDisp,
...
export const YahooAPIStockSelect: React.FC<Props> = (props) => {
    ...
    const [options, setOptions] = useState<YahooStockSearchResult[]>([]);
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        setErrorMessage(null);
        fetchYahooStockSearch(trimmedValue, controller.signal)
            .then((results) => {
                setOptions(results);
            })
            .catch((error) => {
    ...
    return (
        <div className="relative w-full">
            ...
            {errorMessage && !shouldShowDropdown && (
                <p className="mt-1 text-xs text-app-danger">{errorMessage}</p>
            )}
        </div>
    );
};
